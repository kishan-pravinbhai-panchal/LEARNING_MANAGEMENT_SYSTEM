...
const AddCourse = () => {
  ...
  const handleChapter = (action, chapterId) => {
    ...
    setChapter(
      chapter.map((chapter) => chapter.chapterId === chapterId ? {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex flex-col gap-1'>
          <p>Discount %</p>
          <input onChange={e => setDiscount(e.target.value)} value={discount} type="number" placeholder='0' min={0} max={100} className='outline-none md:py-2.5 py-2.5 w-28 px-3 rounded border border-gray-500 ' />
        </div>
